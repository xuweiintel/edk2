...
Error:
  ...
  DEBUG ((DEBUG_ERROR, "FmpDxe: EvaluateDependencies() - RESULT = FALSE (Status = %r)\n", Status));
  return FALSE;
}
  ...
  *IsSatisfied       = TRUE;
  ...
  for (Index = 0; Index < mNumberOfFmpInstance; Index ++) {
    if (mFmpImageInfoBuf[Index] != NULL) {
      ...
      if ((mFmpImageInfoBuf[Index]->AttributesSetting & IMAGE_ATTRIBUTE_DEPENDENCY) &&
  ...
  )
{
  ...
  //
  // Validate the depex and get tis size
  //
  if (ValidateImageDepex (Depex, MaxDepexSize, DepexSize)) {
    return Depex;
  }
