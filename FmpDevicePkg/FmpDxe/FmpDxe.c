  ...
  )
{
  ...
  if (Private->Descriptor.AttributesSetting & IMAGE_ATTRIBUTE_DEPENDENCY) {
    Private->Descriptor.Dependencies = GetFmpDependencyFromFmp (NULL);
  }
  ...
  )
{
  ...
  *ImageUpdatable = IMAGE_UPDATABLE_VALID;
  ...
  //
  // Set to satisfied and then if depednency evluates to false it will update this flag.
  //
  ...
  //
  Dependencies = GetFmpDependencyFromCap ((EFI_FIRMWARE_IMAGE_AUTHENTICATION *)Image, ImageSize, &DependenciesSize);
  ...
  //
  if (IsFmpDependencyCheckRequired()) {
    Status = EvaluateFmpDependencies (Private->Descriptor.ImageTypeId, Version, Dependencies, DependenciesSize, &IsDepexSatisfied);
  ...
  )
{
  ...
  //
  // Finished the FmpDeviceSetImage without error
  // Indicate that starts to save Fmp dependnecy
  //
  Progress (98);
  ...
  //
  if ((Private->Descriptor.AttributesSetting & IMAGE_ATTRIBUTE_DEPENDENCY) && Dependencies != NULL) {
    Status = SaveFmpDependency (Dependencies, DependenciesSize);
    if (EFI_ERROR (Status)) {
      if (Status == EFI_UNSUPPORTED) {
        DEBUG ((DEBUG_INFO, "FmpDxe(%s): SetTheImage() SaveFmpDependency is not supported.\n", mImageIdName));
      } else {
        DEBUG ((DEBUG_ERROR, "FmpDxe(%s): SetTheImage() SaveFmpDependency from FmpDependencyLib failed. Status =  %r.\n", mImageIdName, Status));
      }
    }
    Status = EFI_SUCCESS;
  }
